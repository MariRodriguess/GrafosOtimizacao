...
#GRAFO COM CONEXÕES ESTABELECIDAS ANTES DA APLICAÇÃO DO ALGORITMO DE FLUXO MÁXIMO (de forma manual)

# Criação de um grafo direcionado
G_Ant = nx.Graph()

# Adicionar nós para clientes e antenas
...
# Adicionar origem e destino (artificialmente)
...
print("Grafo 1 - Todas as Conexões Estabelecidas de Forma Manual Antes da Aplicação do Algoritmo de Fluxo Máximo")
plt.show()
...
# Adicionar arestas com os pesos ((plano * (1 / sinal)) * (-1))
...
print("Grafo 2 - Todas as Conexões Possíveis Para Cada Cliente")
plt.show()
    ...
    if conexoes: # Verificar se há conexões para o cliente
        ...
        for con in conexoes_data:
            if con['Cliente'] == cliente and con['Antena'] == antena_max_fluxo:
                escreverArq(cliente, antena_max_fluxo, con['Sinal(dBm)']) #Escrever as conexões de maior fluxo no arquivo "ConexoesResultantes.csv"
...
plt.title('Grafo Resultante com Conexões de Maior Fluxo') 
print("Grafo 3 - Grafo Resultante com as Conexões de Maior Fluxo para Cada Cliente Após a Aplicação do Algoritmo de Fluxo Máximo")
plt.show()
